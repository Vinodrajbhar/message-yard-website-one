...
function initUnifiedMotionAndScrubEngine() {
  ...
  function renderLoop() {
    ...
    // 2. CENTERED NARRATIVE (Bottom to Up)
    if (centeredSec && centeredContainer) {
    ...
    // 3. CAPABILITY CARDS (Bottom to Up on Mobile)
    ...
    // 4. PLATFORM ARCHITECTURE (Bottom to Up on Mobile)
    if (howSec) {
    ...
    // 5. CHANNELS GRID (Bottom to Up on Mobile)
    if (linkSec && channelBoxes.length) {
